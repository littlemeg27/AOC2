...
-(int)birdDestruction:(NSString*)birdSounds; //calculation method
{
    if ([self isLoud] == [self holdBird])
    {
        ...
        return [NSString stringWithFormat:@"The Amazon %@ and has a destrustion rate of %f if held", [self birdNoises], [self birdDestructionRate]];
    }
    else if ([self isLoud] || ![self holdBird])
    {
        [self setBirdDestructionRate:[self birdDestructionRate] + 2 - [self birdToyStrength]];
        return [NSString stringWithFormat:@"The Amazon %@ and has a destrustion rate of %f if let out of the cage to play", [self birdNoises], [self birdDestructionRate]];
    }
    else
    {
        [self setBirdDestructionRate:[self birdDestructionRate] + 5 - [self birdToyStrength]];
        return [NSString stringWithFormat:@"The Amazon %@ and has a destrustion rate of %f if not held and left in the cage", [self birdNoises], [self birdDestructionRate]];
    }
